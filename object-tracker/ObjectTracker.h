...
typedef bool		    BBOOL;
enum IMAGE_TYPE {MD_RGB=0, MD_RGBA, MD_YUV, MD_WAVELET};
...
typedef struct _Rectangle {
	...
	int width_;
	int height_;
	_Rectangle(int x, int y, int width, int height) : x_(x), y_(y), width_(width), height_(height)
	{}
} Rectangle;
...
private:
	...
	void FindNextLocation(UBYTE8 *frame, int iterations = 5);
...
public:
	...
	Rectangle ObjeckTrackerHandlerByUser(UBYTE8 *frame, int iterations = 5);
