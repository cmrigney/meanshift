...
Rectangle CObjectTracker::ObjeckTrackerHandlerByUser(UBYTE8 *frame, int iterations)
{
   m_cActiveObject = 0;
   ...
   if (m_sTrackingObjectTable[m_cActiveObject].Status)
   {
	   ...
	   {
		   FindNextLocation(frame, iterations);    
	   }
   }
...
void CObjectTracker::FindHistogram(UBYTE8 *frame, FLOAT32 (*histogram))
{
 ...
 //Get box
Rectangle CObjectTracker::GetObjectBox()
{
  return Rectangle(m_sTrackingObjectTable[m_cActiveObject].X, m_sTrackingObjectTable[m_cActiveObject].Y, m_sTrackingObjectTable[m_cActiveObject].W, m_sTrackingObjectTable[m_cActiveObject].H);
}
...
void CObjectTracker::FindNextLocation(UBYTE8 *frame, int iterations)
{
  UBYTE8  iteration = 0;
